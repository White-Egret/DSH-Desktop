...
2. Rust computes the child webview's Y offset with the pure function `content_offset_for(mode, is_safe_mode, hidden)` (toolbar height, 43.2px, for pinned / Safe Mode; `0` for auto-hide while collapsed) and syncs its position and size. **The size is always the full client area — only the position follows the mode.** The embedded webview is a native child window, so anything past the parent's client area is clipped by Windows; but if the shell "helpfully" subtracts that 43.2px from the height too, the page reports an `innerHeight` shorter than what is visible, DSH lays out against a smaller viewport, and you get content shrunk into the top-left corner with empty space to the right and below plus a leftover strip of toolbar background at the top. The invariant is pinned by the Rust unit test `content_height_is_the_full_client_height_for_every_mode`;
