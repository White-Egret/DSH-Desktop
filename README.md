...
- **系统托盘常驻**：点窗口 X 不再直接退出，而是最小化到托盘；托盘右键菜单提供「显示主窗口 / 开机自动启动 / 退出 Launcher」，真正退出时才结束 DSH 进程树（`taskkill /PID <pid> /T /F` + Job Object 双保险，程序崩溃也不残留进程）
...
1. 安装/运行 DSH-Launcher（NSIS 安装包或独立 exe 均可），窗口默认 1200×760。
...
```
┌──────────────────────────────────────────────────────────────┐
│ ▶启动 ■停止 ⟳重启 ⤓更新DSH 检查版本 日志 ⚙设置   ● 运行中 · 端口 3000 · 版本 1.0.0 │ ← 43.2px 浅灰工具栏
├──────────────────────────────────────────────────────────────┤
│                                                              │
│   等待中：一行提示 + 转圈（已等待 X 秒…）                        │
│   失败时：一行红色错误                                         │
...
- **托盘图标**（黑鲸鱼）：
  - 左键单击 / 双击 → 显示并聚焦主窗口；
  - 右键 → 菜单：「显示主窗口」「开机自动启动（勾选切换）」「退出 Launcher」。
- **退出 Launcher**（托盘菜单）= 停止 DSH 进程树并退出整个程序；程序崩溃时 Job Object 兜底清理，绝不残留进程。
...
- **关了窗口以为退出了**：点 X 是最小化到托盘，程序和 DSH 都还在后台运行；要真正退出请用托盘右键菜单「退出 Launcher」。
- **开机自启没生效**：确认设置开关已开启（或托盘菜单有勾选）；检查 `reg query "HKCU\Software\Microsoft\Windows\CurrentVersion\Run" /v "DSH Launcher"`；注意免安装 portable exe 若被移动路径需重新开启一次开关。
