...
- **安全模式**：用独立纯净家目录（`%USERPROFILE%\.dsh-safe`）在 3081 端口启动 DSH，检查与修复日常环境；只借用 `.credentials.yaml` 一个文件（内容全程不出 Rust 侧），旧目录进入前归档绝不删除，退出后自动重启日常实例并验证修复结果 —— 见「安全模式」
...
2. **基线重置**（可在首选项关闭）：已存在的 `.dsh-safe` 整目录重命名为 `.dsh-safe-archive-<YYYYMMDD-HHMMSS>` **归档（绝不删除）**，重建空目录 —— 每次进入都是恒定的原厂基线。
...
**退出与修复验证闭环**：「退出安全模式」会 kill 安全实例（Child 句柄在 Tauri State 中，应用退出/窗口销毁同样兜底清理，强杀场景由 Job Object 在内核层结束进程树，绝不留孤儿进程占 3081），然后按日常模式既有路径重启；若日常实例在「修复验证等待」（默认 60 秒，首选项可调，0 = 关闭）内未就绪，提示「修复可能未成功」并提供一键**返回安全模式**。
