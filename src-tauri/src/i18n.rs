...
pub fn t(key: &str) -> &'static str {
    ...
    match key {
        ...
        "err_node_missing" => if en { "Node.js (node.exe) not found.\nDSH requires Node.js: install the LTS from https://nodejs.org, then restart this app and retry." } else { "未找到 Node.js（node.exe）。\nDSH 依赖 Node.js 运行：请先安装 Node.js LTS（https://nodejs.org），装完重启本程序后重试。" },
        // ---------- Node.js 最低版本（DSH 的运行下限，低于它 DSH 起不来） ----------
        // 只有一个 {0} 占位时 fmt 也能正常工作（fmt 是顺序替换，多余的占位不会被用到）
        // 版本过低是**硬拦截**（见 process.rs）：首选项里已不再提供升级/放行入口，
        // 所以文案只指向官方安装包与「重启本程序」——不承诺任何已不存在的界面按钮。
        "err_node_too_old" => if en { "The installed Node.js is too old: detected {0}, but DSH requires {1} or newer.\nDSH will fail to start on this version (cryptic errors or an immediate exit) — it is not a problem with your DSH settings.\nInstall the current LTS from https://nodejs.org (the official installer overwrites the same directory; PATH and your installed DSH are unaffected), then restart this app and retry." } else { "本机 Node.js 版本过低：检测到 {0}，而 DSH 要求 {1} 或更高。\n在这个版本上 DSH 会启动失败（可能报看不懂的错误、或刚起来就退出），这不是 DSH 配置问题。\n请到 https://nodejs.org 安装当前 LTS（官方安装包会覆盖原目录，PATH 与已装的 DSH 都不受影响），装完重启本程序后重试。" },
        "log_node_too_old_block" => if en { "[launcher] Start blocked: Node.js {0} is below the required {1}; DSH cannot boot on it. Upgrade Node.js to the current LTS and restart this app." } else { "[launcher] 已拦截启动：Node.js {0} 低于 DSH 要求的 {1}，DSH 无法在其上启动。请升级 Node.js 到当前 LTS 后重启本程序。" },
        ...
        "setup_node_dir_mismatch" => if en { "The Node.js installer reported success but did not put node.exe into the chosen location {0}. Actually detected: {1}. The official MSI ignored the custom directory. Uninstall Node.js and reinstall it into that folder." } else { "Node.js 安装程序报告成功，但没有把 node.exe 放进指定目录 {0}。实际检测到：{1}。官方 MSI 未采纳自定义目录。请卸载 Node.js 后重新装到该目录。" },
