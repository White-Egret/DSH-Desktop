...
pub fn t(key: &str) -> &'static str {
    ...
    match key {
        ...
        "err_node_missing" => if en { "Node.js (node.exe) not found.\nDSH requires Node.js: install the LTS from https://nodejs.org, or check the paths in Preferences, then retry." } else { "未找到 Node.js（node.exe）。\nDSH 依赖 Node.js 运行：请先安装 Node.js LTS （https://nodejs.org），或打开「首选项」确认路径后重试。" },
        // ---------- Node.js 最低版本（DSH 的运行下限，低于它 DSH 起不来） ----------
        // 只有一个 {0} 占位时 fmt 也能正常工作（fmt 是顺序替换，多余的占位不会被用到）
        "err_node_too_old" => if en { "The installed Node.js is too old: detected {0}, but DSH requires {1} or newer.\nDSH will fail to start on this version (cryptic errors or an immediate exit) — it is not a problem with your DSH settings.\nUpgrade Node.js to the current LTS: open this app's first-run wizard / Preferences and use \"Download and install the official LTS\", or install it yourself from https://nodejs.org.\nIf you deliberately want to keep this Node.js version and start DSH anyway, enable \"Keep this version and start anyway\" in Preferences (DSH will most likely fail — the app will not block you again)." } else { "本机 Node.js 版本过低：检测到 {0}，而 DSH 要求 {1} 或更高。\n在这个版本上 DSH 会启动失败（可能报看不懂的错误、或刚起来就退出），这不是 DSH 配置问题。\n请升级到当前 LTS：在本程序的首次运行向导 / 「首选项」里点「一键下载并安装官方 LTS」，或自行到 https://nodejs.org 安装。\n若你确认要保留该版本并继续尝试启动，可在「首选项」中开启「保留该版本并继续启动」（DSH 很可能仍然失败，程序不会再拦截）。" },
        "log_node_too_old_block" => if en { "[launcher] Start blocked: Node.js {0} is below the required {1}; DSH cannot boot on it. Upgrade Node.js to the LTS, or enable \"Keep this version and start anyway\" in Preferences." } else { "[launcher] 已拦截启动：Node.js {0} 低于 DSH 要求的 {1}，DSH 无法在其上启动。请升级 Node.js 到 LTS，或在「首选项」中开启「保留该版本并继续启动」。" },
        ...
        "log_timeout_stop" => if en { "[launcher] Timed out waiting for DSH ({0} s); stopping the process tree. If DSH cold start really needs longer, raise the timeout in Preferences or set it to 0 (wait forever)." } else { "[launcher] 等待 DSH 就绪超时（{0} 秒），正在停止进程树。若 DSH 冷启动确实很慢，可在首选项中调大超时或设为 0（一直等待）。" },
        "err_timeout" => if en { "Start timeout: DSH was not ready within {0} s; the process has been stopped. Raise the timeout in Preferences, or set 0 to wait forever." } else { "启动超时：DSH 在 {0} 秒内未就绪，进程已停止。可在首选项中调大超时时间，或设为 0 表示一直等待。" },
        ...
        "log_theme_synced" => if en { "[launcher] Appearance synced to DSH settings.yaml (ui-theme.preference={0}); open DSH pages follow it live (no DSH restart needed)." } else { "[launcher] 外观已同步写入 DSH settings.yaml（ui-theme.preference={0}），已打开的 DSH 页面会实时跟随换肤（无需重启 DSH）。" },
        ...
        "err_cfg_prog_danger" => if en { "Unsafe program path \"{0}\": the character \"{1}\" is not allowed (cmd.exe could expand it or break the quoting). Please pick this file again in Preferences." } else { "程序路径「{0}」不安全：含有不允许的字符「{1}」（可能被 cmd.exe 展开或破坏引号配对）。请在「首选项」中重新选择该程序。" },
        // ---------- 配置路径策略（MEDIUM-3，见 config.rs 的 validate_home_dir / validate_program_file） ----------
        "err_path_empty" => if en { "{0}: the path is empty. Fill it in or click \"Auto-detect\" in Preferences." } else { "{0}：路径为空。请在「首选项」中填写，或点击「自动检测」。" },
        ...
        "err_prog_ext" => if en { "{0}: only .exe / .cmd / .bat can be launched (a .ps1 or extension-less file would go through unpredictable file associations). Re-select the program in Preferences." } else { "{0}：只允许启动 .exe / .cmd / .bat 文件（.ps1 或无扩展名会走不可预测的文件关联）。请在「首选项」中重新选择。" },
        "err_prog_missing" => if en { "{0}: no such executable file. Click \"Auto-detect\" in Preferences or pick the path again." } else { "{0}：找不到该可执行文件。请在「首选项」中点「自动检测」或重新选择路径。" },
